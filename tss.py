...
class TSSData:
    ...
    def load_csv(self, file_path):
        print(f"Load TSS data from the {file_path} CSV file.")
    ...
    def fetch_new_data(self, start_date, end_date):
        ...
        activities = self.client.get_activities_by_date(
            start_date.strftime("%Y-%m-%d"), end_date.strftime("%Y-%m-%d"), "running"
        )

        # Ordina le attività in base alla data in ordine crescente
        activities.sort(
            key=lambda x: datetime.strptime(x["startTimeLocal"], "%Y-%m-%d %H:%M:%S")
        )
        ...
        for activity in activities:
            ...
            # Aggiungi le date mancanti con TSS uguale a 0
            while current_date.date() < activity_date.date():
                new_data.append({"date": current_date.strftime("%Y-%m-%d"), "tss": 0})
                current_date += timedelta(days=1)

            # Aggiungi l'attività corrente
            rounded_tss = round(activity["activityTrainingLoad"])
            new_data.append(
                {"date": activity_date.strftime("%Y-%m-%d"), "tss": rounded_tss}
            )
            current_date = activity_date + timedelta(days=1)

        # Aggiungi le date mancanti fino alla data di fine
    ...
    def update_data(self):
        ...
        if today > last_date:
            print(f"Update TSS data to the {today.strftime('%Y-%m-%d')} date.")
    ...
    def save_csv(self, file_path):
        print(f"Save TSS data to the {file_path} CSV file.")
